...
namespace MetadataExtractor.MediaLibraryProcessor
{
    ...
    /// </summary>
    internal sealed class TextFileOutputHandler : FileHandlerBase
    {
            ...
            try
            {
                ...
                try
                {
                    ...
                    foreach (var directory in directories)
                    {
                        ...
                        {
                            var wrote = false;
                            var iterator = new XmpIterator((XmpMeta)xmpDirectory.XmpMeta, null, null, new IteratorOptions { IsJustLeafNodes = true });
                            ...
                            while (iterator.HasNext())
                            {
                                ...
                                writer.Write($"[XMPMeta - {ns}] {path} = {value}\n");
                                wrote = true;
                            }
                            if (wrote)
                                writer.Write('\n');
